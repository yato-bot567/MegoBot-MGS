...
const handler = async (m, { conn, args, usedPrefix }) => {
    ...
    try {
        ...
        if (!text.trim()) {
            await m.reply(`تخيل و انا سأرسل لك ما تخيلته انا كانا استطيع فعلها نعم ! \nمـــثال:\n*.artdraw* girl with hijan  style`);
            return;
        }
        ...
        await conn.sendMessage(m.chat, { image: { url: media }, caption: `https://www.instagram.com/m0hamed.78?igsh=enB0eG95eDh5d2Z1` }, { quoted: m });
    } catch (error) {
        console.error(error);
